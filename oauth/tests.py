...
class OAuthServerTest(TestCase):
    ...
    def test_validate_request_token(self):
        ...
        self.assertTrue(self.oauthserver.validate_request_token(self.clientcredentials.key, self.token.key))
        self.assertFalse(self.oauthserver.validate_request_token('mdwCgQ4kMCRAfO5A7l6pN4QEOePAE4=', self.token.key))
        self.assertFalse(self.oauthserver.validate_request_token(self.clientcredentials.key, 'm7UQ0_n8M0vUNmd'))

    def test_validate_access_token(self):
        self.token = Token.objects.get(pk=2)
        self.assertTrue(self.oauthserver.validate_access_token(self.clientcredentials.key, self.token.key))
        self.assertFalse(self.oauthserver.validate_access_token('mdwCgQ4kMCRAfO5A7l6pN4QEOePAE4=', self.token.key))
        self.assertFalse(self.oauthserver.validate_access_token(self.clientcredentials.key, 'm7UQ0_n8M0vUNmd'))

    def test_validate_redirect_uri(self):
        self.assertTrue(self.oauthserver.validate_redirect_uri(self.clientcredentials.key, self.clientcredentials.callback))
        self.assertFalse(self.oauthserver.validate_redirect_uri('mdwCgQ4kMCRAfO5A7l6pN4QEOePAE4=', self.clientcredentials.callback))
        self.assertFalse(self.oauthserver.validate_redirect_uri(self.clientcredentials.key, 'http://www.example.com/ready'))

    def test_validate_realm(self):
        self.realm = Realm.objects.get(pk=2)
        self.assertEquals(self.oauthserver.validate_realm(self.clientcredentials.key, self.realm.access_token, None, self.realm.name), self.realm.name)
        self.realm = Realm.objects.get(pk=3)
        self.assertEquals(self.oauthserver.validate_realm(self.clientcredentials.key, self.realm.access_token, self.realm.url, None), self.realm.url)

    def test_validate_requested_realm(self):
        self.realm = Realm.objects.get(pk=1)
        self.assertEquals(self.oauthserver.validate_requested_realm(self.clientcredentials.key, self.realm.name), self.realm.name)
        self.assertFalse(self.oauthserver.validate_requested_realm('mdwCgQ4kMCRAfO5A7l6pN4QEOePAE4=', self.realm.name))
        self.assertFalse(self.oauthserver.validate_requested_realm(self.clientcredentials.key, 'wrong_realm_name'))

#    def test_validate_verifier(self):
#        self.token = Token.objects.get(pk=2)
#        self.assertEquals(self.oauthserver.validate_verifier(self.clientcredentials.key, self.token.key, 'dfg'),'dfg')

    def test_get_client_secret(self):
        self.assertEquals(self.oauthserver.get_client_secret(self.clientcredentials.key), self.clientcredentials.secret)
        self.assertFalse(self.oauthserver.get_client_secret('Vc_89DGdxcBShDhXGkDKJuc8='))

    def test_get_request_token_secret(self):
        self.token = Token.objects.get(pk=1)
        self.assertEquals(self.oauthserver.get_request_token_secret(self.clientcredentials.key, self.token.key), self.token.secret)
        self.assertFalse(self.oauthserver.get_request_token_secret('Vc_89DGdxcBShDhXGkDKJuc8=', self.token.key))
        self.assertFalse(self.oauthserver.get_request_token_secret(self.clientcredentials.key, 'QhYz2iCdGS8xYwUegfSUHF'))
...
#        self.username = 'testuser'
#        self.password = 'pass'
#        self.user = User.objects.create_user(self.username, 'test@eyesopen.com', self.password)
#
#        # Object to test on
#        self.clientcredentials = ClientCredential.objects.get(pk=1)
#
...
#    def testStuff(self):
#
#        c = Client(self.clientcredentials.key, callback_uri=self.clientcredentials.callback)
#
#        uri, headers, body = c.sign(u'http://127.0.0.1:8001/initiate/')

        #TODO nonce/timestamp/signature will change
#        self.assertEqual(headers, {u'Authorization': u'OAuth oauth_nonce="110880830699442379541341263567",'
...
#        dc = DjClient()
#        response = dc.post('/oauth/initiate/', HTTP_AUTHORIZATION=headers['Authorization'])
